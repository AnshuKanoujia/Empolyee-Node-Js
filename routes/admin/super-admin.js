...
router.post('/create-sites', [
  ...
  function (req, res) {
    ...
    if (!errors.isEmpty()) {
      ...
      var q = db.query('INSERT INTO jobsites (siteName, description, supervisors, planner, createdBy, createDate, newAddress, latitude, longitude) SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?', [req.body.siteName, req.body.description, req.body.jobSupervisor, req.session.uid, req.session.uid, moment(new Date()).format('YYYY-MM-DD'), req.body.newAddress, req.body.latitude, req.body.longitude], function (err, rows, fields) {
        ...
        db.query('INSERT INTO user_sites (site_id, user_id, user_role) VALUES (?, ?, 3)', [id, req.body.jobSupervisor], function (err) {
...
router.get('/manage-sites', function (req, res) {
  chksession(req, res);
  db.query('SELECT id, siteName, sitesCode, description, newAddress, createDate FROM jobsites', function (err, rows) {
...
router.get('/view-sites/:id', function (req, res) {
  ...
  db.query(`SELECT jobsites.id, jobsites.siteName, jobsites.sitesCode, jobsites.description, jobsites.newAddress, jobsites.createDate,  jobsites.description, users_job.sup_id AS supervisors, CONCAT(firstName, " ", lastName) AS name FROM jobsites  LEFT JOIN users_job ON (jobsites.id = users_job.siteId) LEFT JOIN users ON (users_job.sup_id = users.id)WHERE jobsites.id=?`, id, function (err, rows) {
...
router.get('/edit-sites/:id', function (req, res) {
  chksession(req, res);
  var id = req.params.id;
  db.query('SELECT js.id AS jobsitesId, js.sitesCode, js.siteName, js.supervisors, js.description, js.newAddress, u.firstName FROM jobsites AS js JOIN users AS u ON (js.supervisors = u.id) WHERE js.id = ?', id, function (err, site) {
    db.query('SELECT CONCAT(`firstName`, " ", `lastName`) AS name, id FROM users WHERE role=3', function (err, supervisor) {
      if (err)
        console.error(err.message)
...
router.post('/updateSupervisor', function (req, res) {
  ...
  db.query('UPDATE jobsites SET supervisors=? WHERE id=?', [userId, siteId], function (err, rows) {

    db.query('SELECT user_id FROM user_sites WHERE user_role=3 AND site_id=? AND is_current=1', siteId, function (err, supId) {
      db.query('UPDATE user_sites SET is_current=0 WHERE user_id=? AND site_id=?', [supId[0].user_id, siteId], function (err) {
        db.query('INSERT INTO user_sites (site_id, user_id, user_role) VALUES (?, ?, 3)', [siteId, userId], function (err, rows) {

          db.query('UPDATE jobsites SET supervisors=? WHERE id=?', [supId[0].user_id, siteId], function (err) {
...
router.get('/view-supervisor', function (req, res) {
  ...
  db.query('SELECT CONCAT(`firstName`, " ", `lastName`) AS name, u.id, u.eCode, u.dob, u.email, u.jobtype, u.skills, u.experience, jt.type_name, (SELECT siteName FROM jobsites WHERE id=(SELECT site_id FROM user_sites WHERE user_id=u.id AND is_current =1)) as siteName, (SELECT count(id) FROM supervisor_taskreporting WHERE status=0 AND supId=u.id) as pending_reports FROM users AS u LEFT JOIN jobtype AS jt ON(u.jobType=jt.id)  where role=3', function (err, rows) {
...
router.get('/submitted-reports/:id', function (req, res) {
  ...
  db.query('SELECT st.*, u.firstName, s.statusName FROM supervisor_taskreporting AS st join users AS u JOIN statusname AS s ON(s.id=st.status) where st.plannerId=? AND st.supId = u.id AND active=1 AND st.status=0 AND st.supId=? ORDER BY st.date DESC', [req.session.uid, id], function (err, rows) {
    if (err)
      console.error(err.message)
...
router.post('/searchUsers', function (req, res) {
  ...
  db.query('SELECT jobTypeId, latitude as site_latitude,longitude as site_longitude FROM jobs AS j JOIN jobsites AS js ON (js.id = j.siteId) where j.id=?', jobId, function (err, rows) {
    ...
    db.query('SELECT id AS designationId, designation_name FROM designation WHERE job_type=?', jobtype, function (err, designation) {
      db.query(`SELECT CONCAT(firstName,' ',lastName) AS name, u.id, u.experience, u.profileImg,u.newAddress, u.latitude, u.longitude, u.ratings, u.latitude, u.longitude, count(u.id) as rank, (SELECT CONCAT(hourly_rate,'/',max_pertime_rate) FROM user_rates AS ur WHERE ur.userId =u.id GROUP by ur.userId) AS rates FROM users u JOIN user_rates AS ur ON (u.id = ur.userId) WHERE firstName LIKE '%${req.body.search ? req.body.search : '%'}%' ${_designationFilter} ${_ratingFilter} AND u.jobType=(SELECT jobTypeId FROM jobs WHERE id ='${jobId}') AND u.role=1 AND u.latitude IS NOT NULL AND u.latitude != '' AND u.search=1 GROUP BY u.id order by rank desc`, [jobId], function (err, users) {
        var cart = req.params.id;
        var site_latitude = parseFloat(rows[0].site_latitude);
        var site_longitude = parseFloat(rows[0].site_longitude);
        distDiffer = [];
...
router.get('/userCart', function (req, res, next) {
  ...
  if (cartCount == 0) {
    ...
    db.query(`SELECT users.id, users.profileImg, CONCAT(firstName,' ',lastName) AS firstName from users where users.id IN ` + list, function (err, users) {
      ...
      db.query('SELECT ur.hourly_rate, ur.max_pertime_rate, d.designation_name, d.id AS designationID FROM user_rates AS ur LEFT JOIN user_designation AS ud ON (ur.userId=ud.user_id) LEFT JOIN designation AS d ON (ud.designation_id=d.id) WHERE ur.userId IN' + list, function (err, rates) {
...
router.get('/view-technician', function (req, res) {
  chksession(req, res);
  db.query('SELECT CONCAT(`firstName`, " ", `lastName`) AS name, users.id, users.eCode, users.experience, users.status, jobtype.type_name AS jobtype FROM users LEFT JOIN jobtype ON(users.jobType=jobtype.id) where role=1', function (err, rows) {
...
router.post('/search_userSkills', function (req, res) {
  chksession(req, res);
  if (req.body.id) {
    db.query('SELECT CONCAT(`firstName`, " ", `lastName`) AS name, users.id, users.eCode, users.experience, users.status, jobtype.type_name AS jobtype FROM users LEFT JOIN jobtype ON(users.jobType=jobtype.id) where users.jobType=? AND role=1', req.body.id, function (err, rows) {
...
router.get('/technicianMap-view', function (req, res) {
  chksession(req, res);
  db.query(`SELECT CONCAT(firstName,' ',lastName) AS name, u.id, u.newAddress, u.profileImg, u.phone_number, u.ratings, u.role, u.latitude, u.longitude, j.type_name, (SELECT designation_name FROM designation WHERE id=(SELECT designation_id from user_designation WHERE user_id = u.id)) as designation, s.total_score, s.neg_mark, s.wrong_answer_count From users AS u LEFT JOIN jobtype AS j ON(u.jobType = j.id) LEFT JOIN results AS s ON(u.id = s.userId)WHERE u.latitude IS NOT NULL AND u.latitude != '' AND u.role=1 AND u.status=1 GROUP by u.id`, function (err, usersListDetail) {
...
router.post('/addUsersToCart', function (req, res) {
  ...
  db.query('SELECT id, firstName, experience, profileImg from users where id=?', id, function (err, users) {
    ...
    req.session.cart = cart;
    req.flash('msg_info', "Selected users added into cart !!");
    res.send('success');
    // res.redirect('/admin/project-planner/userMap-view/' + jobId);
...
router.get('/chat', function (req, res) {
  ...
  db.query(`SELECT msg_from FROM tech_messages WHERE msg_to=?`, uid, function (err, id) {
    ...
    db.query(`SELECT CONCAT(firstName,' ',lastName) AS name, u.profileImg, tm.id, tm.msg_from, tm.msg_to, tm.message, tm.time FROM tech_messages AS tm JOIN users AS u ON(tm.msg_from=u.id) WHERE msg_from IN (?,?) AND msg_to IN (?,?) AND tm.active=1 ORDER BY tm.time LIMIT 1`, [uid, hrId, uid, hrId], function (err, message) {
      ...
      db.query(`SELECT MAX(id) AS id, msg_to, msg_from FROM tech_messages WHERE msg_to=?`, uid, function (err, lastId) {
